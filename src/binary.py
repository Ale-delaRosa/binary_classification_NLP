...
def train_imdb_model():
    """
    Function to train a binary classification model on the IMDB dataset.
    Returns the trained model and test data.
    """
    # Load IMDB data
    (train_data, train_labels), (test_data, test_labels) = imdb.load_data(num_words=10000)

    # Get the word index and decode a sample review
    word_index = imdb.get_word_index()
    reverse_word_index = {value: key for key, value in word_index.items()}
    decoded_review = ' '.join([reverse_word_index.get(i - 3, '?') for i in train_data[0]])

    print("First decoded review:")
    print(decoded_review)
    print("First review label (1=Positive, 0=Negative):", train_labels[0])

    # Function to perform one-hot encoding of reviews
    ...
    print("\nModel results on the test set (Loss, Accuracy):", results)
